...
impl Row {
    ...
    fn new(cells: Vec<Cell>) -> Row {
        ...
        for i in 0..cells.len() {
            if max_height < cells[i].height_of_cell {
                max_height = cells[i].height_of_cell;
            }
            total_width += cells[i].width_of_cell;
        }
       
        Row {
            cells,
            height_of_row:  max_height,
            width_of_row: total_width,
        }
    }
...
impl Table {
    ...
    fn new(rows: Vec<Row>) -> Table {
        ...
        for i in 0..rows.len() {
            table_height += rows[i].height_of_row;
            table_width += rows[i].width_of_row;
        }
        Table {
            rows,
            height_of_table: 0,
